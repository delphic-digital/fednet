...
exports.nunjucks.render = function(str, options, fn) {
  ...
  if(options.settings){
    var env = engine.configure(options.settings.views);

    if(options.settings.filters){
      for (var filter in options.settings.filters) {
        env.addFilter(filter, options.settings.filters[filter]);
      }
    }
